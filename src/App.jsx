...
const App = () => {
  ...
  const componentValues = useRef([]);

  useEffect(() => {
    fetch(`${process.env.PUBLIC_URL}/components.json`)
      .then(response => response.json())
      .then(data => {
        componentValues.current = componentValues.current.slice(0, data.components.length);
        ...
        {componentList.map((component, index) =>
          <div key={index}>
            <Component
              ...
              setValue={v => componentValues.current[index] = v}
            ></Component>
          </div>
        )}
      </div>
